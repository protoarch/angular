...
export class AuthService<T extends User> {
  ...
  constructor(
    ...
    this.opts = options || {
      userType: <new () => T>AUTH_OPTIONS_DEFAULTS.userType,
    };
    this.opts.userType =
      this.opts.userType || <new () => T>AUTH_OPTIONS_DEFAULTS.userType;
    this.opts.tokenName =
      this.opts.tokenName || AUTH_OPTIONS_DEFAULTS.tokenName;
  ...
  login(login: string, password: string): Observable<T> {
    return this.authApi.login(login, password).pipe(
      map((token) => {
        if (token) {
          const tokenValue = token[<string>this.opts.tokenName];
          if (tokenValue) {
            this.authTokenService.saveToken(tokenValue);
          }
        }
  ...
  authorize(requiredClaims: string | string[] | null = null) {
    ...
    if (currentUser.claims) {
      ...
      requiredClaims =
        requiredClaims instanceof Array ? requiredClaims : [requiredClaims];
  ...
  getUser(): T {
    ...
    try {
      ...
      return Object.freeze(
        Object.assign(
          this.opts.userType ? new this.opts.userType() : new User(),
          decoded
        )
      );
    } catch (e) {
